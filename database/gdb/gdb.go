...
type DB interface {
	...
	// The args are for interface{} placeholder parameters in the query.
	...
	GetArray(ctx context.Context, sql string, args ...interface{}) (Array, error)
	...
	// PingSlave checks if interface{} slave database node is accessible.
	...
	// It returns the appropriate LocalType and interface{} conversion errors.
...
// Note that, interface{} operation using `Link` will have no SQL logging.
type Link interface {
