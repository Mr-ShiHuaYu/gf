...
type Model struct {
	...
	fields         []interface{}     // Operation fields, multiple fields joined using char ','.
	fieldsEx       []interface{}     // Excluded operation fields, it here uses slice instead of string type for quick filtering.
	...
	shardingValue  interface{}       // Sharding value for sharding feature.
}
...
func (m *Model) Clone() *Model {
	...
	*newModel = *m
	...
	if n := len(m.fields); n > 0 {
		newModel.fields = make([]interface{}, n)
		copy(newModel.fields, m.fields)
	}
	if n := len(m.fieldsEx); n > 0 {
		newModel.fieldsEx = make([]interface{}, n)
...
// Note that it makes sense only if there's interface{} slave node configured.
