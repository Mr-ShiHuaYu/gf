...
func (c *AdapterRedis) Set(ctx context.Context, key interface{}, value interface{}, duration time.Duration) (err error) {
	...
	if value == nil || duration < 0 {
		...
		if duration == 0 {
			...
			_, err = c.redis.Set(ctx, redisKey, value, gredis.SetOption{TTLOption: gredis.TTLOption{PX: gconv.PtrInt64(int64(duration / time.Millisecond))}})
		}
	}
	return err
}
...
func (c *AdapterRedis) SetIfNotExist(ctx context.Context, key interface{}, value interface{}, duration time.Duration) (bool, error) {
	...
	if ok && duration > 0 {
		// Set the expiration.
		_, err = c.redis.PExpire(ctx, redisKey, int64(duration/time.Millisecond))
		if err != nil {
			return ok, err
		}
		return ok, err
